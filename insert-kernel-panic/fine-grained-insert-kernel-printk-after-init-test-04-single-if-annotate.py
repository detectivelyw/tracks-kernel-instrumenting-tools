                    ...
                    for node_type in ast_info[str(src_line_num)]:
                      if "IfStatement" in node_type:
                        if "{" not in src_line:
                          is_if_single = 0
                        if is_inside_function == 1:
                          output_file.write("[src_line " + str(instrumented_src_line_num) + " *** popular line *** <IfStatement> (InFunction)] " + src_line)
                        else:
                          output_file.write("[src_line " + str(instrumented_src_line_num) + " *** popular line *** <IfStatement> ] " + src_line)
                        # output_file.write(src_line)
                        is_expression = 1
                        break
                    ...
                    for node_type in ast_info[str(src_line_num)]:
                      if "IfStatement" in node_type:
                        if "{" not in src_line:
                          is_if_single = 0
                        if is_inside_function == 1:
                          output_file.write("[src_line " + str(instrumented_src_line_num) + " *** unpopular line *** <IfStatement> (InFunction)] " + src_line)
                        else:
                          output_file.write("[src_line " + str(instrumented_src_line_num) + " *** unpopular line *** <IfStatement> ] " + src_line)
                        # output_file.write(src_line)
                        is_expression = 1
                        break
                      if "Expression" in node_type:
                        if is_inside_function == 1:
                          if is_if_single == 1:
                            output_file.write("[src_line " + str(instrumented_src_line_num) + " *** unpopular line *** <Expression> (InFunction)(IfSingle)] " + src_line)
                          else:
