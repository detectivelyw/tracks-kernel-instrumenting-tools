    ...
    for lines in gcov_data_file:
      if ("SF:" in lines):
        ...
        if lines != "\n":
          ...
          printk_header = "#include <linux/printk.h> \n"
          ...
          for src_line in srcfile: 
            if (is_unpopular_function == 1) and (src_line == "{\n"):
              ...
              instrumented_src_line_num += 1
              output_file.write("	early_printk(" + "\"We reached unpopular paths in " + src_path[len(src_root_path):] + ": line " + str(instrumented_src_line_num) + " \\n\"); \n")
              instrumented_src_line_num += 1
              output_file.write("	panic(" + "\"We reached unpopular paths in " + src_path[len(src_root_path):] + ": line " + str(instrumented_src_line_num) + " \\n\"); \n")
              total_panic_inserted += 1
              print str(total_panic_inserted) + " early_printk() calls inserted"
              src_line_num += 1
              instrumented_src_line_num += 1
              is_unpopular_function = 0
              continue     
