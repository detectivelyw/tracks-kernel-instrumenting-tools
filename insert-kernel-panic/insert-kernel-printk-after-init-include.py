    ...
    for lines in gcov_data_file:
      if ("SF:" in lines):
        ...
        if lines != "\n":
          ...
          for src_line in srcfile: 
            if '#include' in src_line and include_in_src == 0:
              include_in_src = 1
            if "#include <linux/printk.h>" in src_line and include_in_src == 1:
              has_printk_header = 1
            if "#include <linux/kernel.h>" in src_line and include_in_src == 1:
              has_kernel_header = 1
            if '#include' not in src_line and include_in_src == 1:
              include_in_src = 2
              if has_kernel_header == 0:
                panic_header_011 = "#ifndef _KERNEL_INIT_DONE_ \n"
                output_file.write(panic_header_011)
                instrumented_src_line_num += 1 
                panic_header_012 = "#define _KERNEL_INIT_DONE_ \n"
                output_file.write(panic_header_012)
                instrumented_src_line_num += 1
                panic_header_013 = "extern bool kernel_init_done; \n"
                output_file.write(panic_header_013)
                instrumented_src_line_num += 1
                panic_header_014 = "#endif \n"
                output_file.write(panic_header_014)
                instrumented_src_line_num += 1
              if has_printk_header == 0:
                panic_header_02 = "int printk(const char *fmt, ...); \n"
                output_file.write(panic_header_02)
                instrumented_src_line_num += 1
              has_kernel_header = 0
              has_printk_header = 0
            ...
            if (is_unpopular_function == 1) and (src_line == "{\n"):
              ...
              instrumented_src_line_num += 1
              output_file.write("	if (kernel_init_done) printk(" + "\"We reached unpopular paths in " + src_path[len(src_root_path):] + ": line " + str(instrumented_src_line_num) + " \\n\"); \n")
